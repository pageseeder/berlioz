...
/**
 * Describes how a direct HTTP caller may prove it is allowed to invoke Berlioz control
 * parameters (e.g. {@code berlioz-reload}).
 *
 ...
 *
 * <p>This is the "direct channel" of {@link BerliozConfig#hasControl(javax.servlet.http.HttpServletRequest)};
 * see that method for how it combines with the delegated (request-attribute) channel.
 *
 * <p>The string representation (lower-case enum name) is the value used in the
 * {@code berlioz.control.access} configuration property.
 *
...
public enum ControlAccess {
  ...
  /** The direct channel never grants access. Safe default. */
  OFF,
  ...
  /** Grants access when the caller supplies a matching {@code Authorization: Berlioz <key>} header. */
  KEY;
   ...
   *
   * @param value the raw config string (e.g. {@code "off"}, {@code "loopback"}, {@code "lan"}, {@code "key"})
   * @return the matching mode, or {@link #OFF} if the value is not recognised
   */
  public static ControlAccess parse(String value) {
