...
/**
 * A utility class providing simple static methods to generate SHA-512 hash values.
 *
 * <p>A SHA-512 hash is expressed as a 128-digit hexadecimal number.
 * SHA-512 is available on all common JVM distributions.
 *
 * <p>Delegates to {@link Hashes} using {@link Hashes.Algorithm#SHA_512}.
 *
...
public final class SHA512 {
   ...
   *
   * @return The SHA-512 hash as a 128-character lowercase hexadecimal string.
   */
  public static String hash(String text) {
    return Hashes.hash(text, Hashes.Algorithm.SHA_512);
  }
  ...
  public static String hash(byte[] data) {
    return Hashes.hash(data, Hashes.Algorithm.SHA_512);
  }
  ...
  public static String hash(InputStream in) throws IOException {
    return Hashes.hash(in, Hashes.Algorithm.SHA_512);
  }
  ...
  public static String hash(File file) throws IOException {
    return Hashes.hash(file, Hashes.Algorithm.SHA_512);
  }
  ...
  public static String hash(File file, boolean strong) throws IOException {
    return Hashes.hash(file, strong, Hashes.Algorithm.SHA_512);
  }
  ...
  public static String hash(Path path) throws IOException {
    return Hashes.hash(path, Hashes.Algorithm.SHA_512);
  }
  ...
  public static String hash(Path path, boolean strong) throws IOException {
    return Hashes.hash(path, strong, Hashes.Algorithm.SHA_512);
  }
